...
#[napi]
pub fn vrf_verify(public_key: String, message: String, proof: String, aux_data: Option<String>) -> Result<bool> {
    ...
    let proof_bytes = hex::decode(&proof)
        .map_err(|e| Error::from_reason(format!("Invalid proof hex: {}", e)))?;

    let proof: ark_vrf::ietf::Proof<BandersnatchSha512Ell2> = ark_vrf::ietf::Proof::deserialize_compressed(&*proof_bytes)
...
pub fn vrf_proof_to_hash(proof: String) -> Result<String> {
    ...
    proof.serialize_compressed(&mut proof_hash)
